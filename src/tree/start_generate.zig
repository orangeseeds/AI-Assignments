...
pub fn AStarGeneration(sst: *SST, allocator: *Allocator, goal: State, limit: i32) !void {
...
test "test_sst_a_star" {
    ...
    try AStarGeneration(&sst, &allocator, goal, 100);
    ...
    while (iter.next()) |node| {
        if (node.parent) |parent| {
            if (node.state.eight_puzzle.status != EightPuzz.StateType.Duplicate) {
